...
export default new Vuex.Store({
    ...
    state: {
        ...
        // 工程
        works: [],
        infoId: { calendarId: '', lineId: '', columnStatus: '', staffId: '' },
        memoBefore: '',
        workBefore: '',
        beforeDate: [],
    ...
    getters: {
        ...
        getAddMemberModalActive: state => state.isAddMemberModalActive,
        getDeleteMemberModalActive: state => state.isDeleteMemberModalActive,
    ...
    mutations: {
        ...
        async fetchCharges(state) {
            ...
            await axios.get('/api/charges', {})
                .then(result => {
                    console.log("=========================")
                    console.log(result);
                    var data_temp = result.staffs;
                    data_temp.sort((a,b) => a.name < b.name)
                    state.managerLists = data_temp;
                    state.chargeLists = result.charges
                    var orders = result.staffs.map(item => {
                        return item.order
                    })
                    ...
                    for (let i = 0; i < count; i++) {
                        if (orders.includes(i + 1)) {
                            ...
                            if (undefinedValue) {
                                ...
                                console.log("未定")
                                console.log(state.managerLists)
                                // state.managerLists.splice(i, 0, { name: "未定", order: (i + 1) });
                                state.managerLists.splice(state.managerLists.length, 0, { name: "未定", id: 0, order: (state.managerLists.length + 1) });
                                console.log(state.managerLists)
                                undefinedValue = true
                            }
                        }
            ...
            for (let i = 0; i < state.calendars.length; i++) {
                ...
                for (let timeType = 0; timeType < 3; timeType++) {
                    ...
                    for (let j = 0; j < state.managerLists.length; j++) {
                        let filterMemberId = state.managerLists[j].id;
                        ...
                        if (state.managerLists[j].name == "未定") {
                            tempProjects[j] = projects.filter(project => {
                                return (project.work_on_date === filterDate) &&
                                    (project.staff === null || project.staff == 0) &&
                            ...
                            tempProjects[j] = projects.filter(project => {
                                if(project.staff){
                                    return (project.work_on_date === filterDate) &&
                                    (project.staff.id === state.managerLists[j].id) &&
                                    (project.time_type === filterTimeType);
                                }else{
                                    return false;
                                }
                        ...
                        tempRemarks[j] = chargeRemarks.filter(chargeRemark => {
                            return (chargeRemark.work_on_date === filterDate) &&
                                (chargeRemark.staff ? chargeRemark.staff.id === filterMemberId : (state.managerLists[j].name == "未定")) &&
                        ...
                        for (let z = 0; z < state.managerLists.length; z++) {
                            ...
                            if (project) {
                                ...
                                let tempProject = {
                                    ...
                                    projectColor: project.project_color_id,
                                    ...
                                    userName: project.orderer.president, // 元請け代表名
                                    userCompany: project.orderer.company, // 元請け会社名
                                    chargeName: chargesName, // 担当者名
                                    staff: staff,
                                    chargeId: chargeId,
                                    projectWorker: project.worker_id,
                                    projectRemark: project.remark,
                                    projectOrdererId: project.orderer.id,
                                }
                                if(columns[j]){
        ...
        */
        ...
        // in process page, click 担当者追加 button to open modal
        toggleAddMemberModal(state) {
            state.isAddMemberModalActive = !state.isAddMemberModalActive;
            Vue.set(state, 'refreshVal', state.refreshVal + 1);
        },
        // in process page, click 担当者追加 button to open modal
        toggleDeleteMemberModal(state) {
            state.isDeleteMemberModalActive = !state.isDeleteMemberModalActive;
            Vue.set(state, 'refreshVal', state.refreshVal + 1);
        },
        // モーダル表示する案件をセット
        showWork(state, columnStatus) {
            // 営業担当者、施工日、時間タイプをセットする
            state.project.charge_id = 0;
            // if(columnStatus !== 'yetcolumns') {
            //     // 選択したモーダルの箇所が「案件調整」の場合
            console.log(state.project.charge_id)
            state.project.worker_id = 0;
            // } else {
        ...
        showMemo(state) {
            ...
            if (state.calendars[state.infoId.calendarId][state.infoId.columnStatus][state.infoId.lineId].memos[state.infoId.staffId]) {
                state.content = state.calendars[state.infoId.calendarId][state.infoId.columnStatus][state.infoId.lineId].memos[state.infoId.staffId].content;
                state.remarkId = state.calendars[state.infoId.calendarId][state.infoId.columnStatus][state.infoId.lineId].memos[state.infoId.staffId].remarkId;
            } else {
                state.content = '';
                state.remarkId = null;
            }
            ...
            // state.workInfo.staffId = staffId;
        },
        ...
        */
        // 架設登録
        registerWork(state, newProject) {
            state.project = { ... state.project, ...newProject}
                /*
                　　元請け情報を取得する
                */
                // 登録した現場の元請け会社ID
            ...
            */
            ...
            let targetProjectStaff = state.managerLists.find((object) => {
                return (object.id === state.project.worker_id);
            });
            ...
            */
            ...
            for (var i = 0; i < state.work_on_array.length; i++) {
                ...
                let projectCharge = state.project.charge_id ? state.project.charge_id : 0;
                let projectWorker = state.project.worker_id ? state.project.worker_id : 0;
                ...
                let projectColor = state.project.project_color_id; // 色
                let userCompany = null;
                // 元請け会社の新規の場合・既存の場合で三項演算子
                if (state.orderer.company) {
                    // 新規の場合
                    userCompany = state.orderer.company;
                } else {
                    // 既存の値からの場合
                ...
                // 元請け会社名
                let staffName;
                if(targetProjectStaff){
                    staffName = targetProjectStaff.name; // 担当者名
                }else{
                    staffName = "未定"; // 担当者名
                }
                projects[i] = {
            ...
            state.project.staff_id = state.managerLists[state.infoId.staffId].id
                ...
                .then(result => {
                    ...
                    */
                    for (let i = 0; i < projects.length; i++) {
                        ...
                        if ((targetProject[0][state.infoId.columnStatus][state.infoId.lineId].works[state.infoId.staffId]) ||
                            (targetProject[0][state.infoId.columnStatus][state.infoId.lineId].memos[state.infoId.staffId])) {
                            state.infoId.lineId++;
                        ...
                        // 案件情報を登録する
                        targetProject[0][state.infoId.columnStatus][state.infoId.lineId].works[state.infoId.staffId] = projects[i];
                        // 案件にIDを指定
                        targetProject[0][state.infoId.columnStatus][state.infoId.lineId].works[state.infoId.staffId].projectId = result.id;
                        Vue.set(state, 'refreshVal', state.refreshVal + 1);
                    }
                })
                .catch(result => {
        ...
        // 架設移動
        sendWorkId(state, { calendarId, lineId, staffId, columnStatus }) {
            state.workInfo.calendarId = calendarId;
            state.workInfo.lineId = lineId;
            state.workInfo.columnStatus = columnStatus;
            state.workInfo.staffId = staffId;
        ...
        */
        ...
        registerMemo(state, { infoId, content }) {
            ...
            state.calendars[infoId.calendarId][infoId.columnStatus][infoId.lineId].memos[infoId.staffId] = {
                content: content
            };
            let timeType = 0;
            ...
            let params = {
                // charge_id: state.managerLists[infoId.staffId].id,
                worker_id: state.managerLists[infoId.staffId].id,
                time_type: timeType,
            ...
            if (!state.remarkId) {
                ...
                axios
                    ...
                    .then(result => {
                        ...
                        // 返却されたIDをcalendarsの値へ返す
                        state.calendars[infoId.calendarId][infoId.columnStatus][infoId.lineId].memos[infoId.staffId].remarkId = result.id;
                    })
                    // 失敗した場合
                    .catch(result => {
                        // 失敗アラート
                ...
                axios
                    ...
                    .then(result => {
                        console.log('メモの更新完了');
                        state.calendars[infoId.calendarId][infoId.columnStatus][infoId.lineId].memos[infoId.staffId].remarkId = state.remarkId;
                    })
                    .catch(result => {
        ...
        // Add Member
        registerMember(state, { name, password, phone, edit_type }) {
            ...
            .then(result => {
                 ...
                 * Add manager list
                 *
                 * Here, you have to add the staff (if he is editor) to state.managerLists
                 */
                if (result.data == "success") {
                    state.managerLists.splice(state.managerLists.length - 1, 0, {
        ...
        deleteMember(state, {id}) {
            ...
            .then(result => {
                console.log(result)
                state.managerLists = state.managerLists.filter(function( obj ) {
                    return obj.id !== id;
                });
            })
        },
        // メモ：移動
        sendid(state, { calendarId, lineId, staffId, columnStatus }) {
            state.infoId.calendarId = calendarId;
            state.infoId.lineId = lineId;
            state.infoId.columnStatus = columnStatus;
            state.infoId.staffId = staffId;
        },
        ...
        addWork(state, { id, timeStatus, lineNumber, target }) {
            ...
            let projectParams = {
                ...
                staff_id: state.managerLists[target].id ? state.managerLists[target].id : '0',
            };
        ...
        changeManagerList(state, val) {
            ...
            const value_purpose = state.managerLists[val.newIndex];
            ...
            if (afterSort == state.managerLists.length) {
                alert("[未定]の位置を変更することはできません。")
                return;
            }
            if (currentSort == state.managerLists.length) {
                alert("[未定]の位置を変更することはできません。")
                return;
            }
            if (afterSort == currentSort) {
                alert('変化はありません。')
                return;
            }
    ...
    actions: {
        // 工程表の現在地を送る
        sendid({ commit }, { calendarId, lineId, staffId, columnStatus }) {
            commit('sendid', { calendarId, lineId, staffId, columnStatus });
        },
        // メモ登録
        ...
        registerMember(context, { name, password, phone, edit_type }) {
            context.commit('registerMember', { name, password, phone, edit_type });
        },
        deleteMember(context, {id}) {
            context.commit('deleteMember', { id });
        },
        setResetFlg(state, val) {
            state.reset_flg = val;
        },
        ...
        // 架設登録
        registerWork(context, project) {
            context.commit('registerWork', project);
        },
