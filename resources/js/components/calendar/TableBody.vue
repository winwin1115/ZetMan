                    ...
                    <!-- <div class="process-table-tr work-field amcolumns" v-for="(amcolumn, lineNumber) in calendar.amcolumns" :key="`first-${lineNumber}`"> -->
                        ...
                        <div class="process-table-td" v-for="(manager, target) in managers" :key="manager.target">
                            <div class="work-field__work flex__wrap f__center v__center">
                                ...
                                >
                                    <div class="field__work__content"
                                        v-if="amcolumn.works[target]"
                                        @click="showWorkDetail(id, lineNumber, target, 'amcolumns')">
...
<script>
	...
	export default {
        ...
        updated:function() {
            this.$store.watch(
                (state)=>{
                    return this.$store.state.managerLists // could also put a Getter here
                },
                (newValue, oldValue)=>{
                    //something changed do something
                    if (oldValue == this.managers || (oldValue.length -1 == newValue.length)) {
		...
		methods: {
            ...
            // 登録モーダル
			// メモのモーダルをオープンする
			openMemoModal: function(calendarId, lineId, staffId, columnStatus) {
				this.$store.dispatch('sendid', {calendarId, lineId, staffId, columnStatus});
				this.$store.commit('showMemo');
				this.$store.commit('toggleMemoModal');
			},
			// 工程のモーダルをオープンする
			openWorkModal: function(calendarId, lineId, staffId, columnStatus) {
				this.$store.dispatch('sendid', {calendarId, lineId, staffId, columnStatus});
				this.$store.commit('showWork', columnStatus);
				this.$store.commit('toggleWorkModal');
            },
            // 工程の変更モーダルをオープンする
            editProject: function(calendarId, lineId, staffId, columnStatus) {
                this.$store.dispatch('sendid', {calendarId, lineId, staffId, columnStatus});
