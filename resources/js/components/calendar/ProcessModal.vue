<template>
	<div class="modal">
        <!-- 工程表Add Member modal -->
		<div class="memo-modal modal-wrap content__wrap" :class="{'is-open': this.$store.getters.getAddMemberModalActive}" @click.self="closeAddMemberModal">
                            ...
                            <div class="content__box">
                                    ...
                                    <div class="content__input">
                                        ...
                                        <div class="input__wrap">
                                            <div class="input__box">
                                                <input class="bgType" type="text" name="name" v-model="newMember.name" required />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="content__submit f__center">
                                    <div class="submit__box">
                                        <input class="clickonce" type="submit" value="スタッフ登録" />
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="memo-modal modal-wrap content__wrap" :class="{'is-open': this.$store.getters.getDeleteMemberModalActive}" @click.self="closeDeleteMemberModal">
                    ...
                    <div class="content__floar">
                        <div class="content__floar__inner">
                            <div class="content__box">
                                <div class="content__box__inner">
                                    <div class="content__input">
                                        <div class="headline attention must">スタッフ選択</div>
                                        <div class="input__wrap">
                                            <div class="input__box">
                                                <select class="bgType" name="" v-model="deleteMemberId" required >
                                                    <template v-for="(charge, index) in charges_for_delete">
                                                        <option v-if="charge.id != 0" :label="charge.name" :key="charge.id" :value="charge.id">{{charge.name}}</option>
                                                    </template>
                                                </select>
                                            </div>
                                        </div>
                                ...
                                <div class="content__submit f__center">
                                    <div class="submit__box">
                                        <input class="clickonce" type="submit" value="スタッフ削除" />
                                    </div>
                                </div>
                            </div>
                        </div>
		...
		<div class="work-modal modal-wrap content__wrap" :class="{'is-open': this.$store.getters.getWorkModalActive}" @click.self="closeWorkModal">
			<form @submit.prevent="registerWork">
				<div class="content__floar">
					...
					<div class="content__input">
						...
						<div class="input__wrap">
							<div class="input__box selectBox">
                                <select class="bgType" name="" v-model="loginId" required v-if="isCharge">
									<template v-for="(charge, index) in origin_charges">
									...
									<!-- <option v-for="(charge, index) in charges" :key="charge.id" :value="charge.id">{{charge.name}}</option> -->
											<option ::label="charge.name" :key="index" :value="charge.id" v-if="charge.edit_type == 0" >{{charge.name}}</option>
									</template>
                                    <option value="0">未定</option>
								</select>

								<select class="bgType" name="" v-model="project.charge_id" required v-else>
									<template v-for="(charge, index) in origin_charges">
									<!-- <template v-for="item in charges"> -->
										<!-- <option :label="item.name" :value="item.id" :key="item.name">{{ item.name }}</option> -->
									<!-- <option v-for="(charge, index) in charges" :key="charge.id" :value="charge.id">{{charge.name}}</option> -->
					...
					<div class="content__input">
						...
						<div class="input__wrap">
							<div class="input__box">
								...
								<!-- <input class="bgType" type="tel" name="" v-model="project.tel"> -->
                                <input class="bgType" type="tel" name="" v-model="project.tel" oninput="value = value.replace(/[０-９]/g,s => String.fromCharCode(s.charCodeAt(0) - 65248)).replace(/\D/g,'');">
							</div>
						</div>
					</div>
					<div class="content__input">
					...
					<div class="content__input">
						<div class="headline attention any">道路規制</div>
					...
					<div class="content__input">
						...
						<div class="input__box">
							...
							<ul class="flex__wrap selectColor">
								<li class="colorBox one"><input type="radio" ref="color1" v-model="project.project_color_id" value="1" name="colorGroup" @click="radioDeselection(1)"><label></label></li>
								<li class="colorBox two"><input type="radio" ref="color2" v-model="project.project_color_id" value="2" name="colorGroup" @click="radioDeselection(2)"><label></label></li>
								<li class="colorBox three"><input type="radio" ref="color3" v-model="project.project_color_id" value="3" name="colorGroup" @click="radioDeselection(3)"><label></label></li>
								<li class="colorBox four"><input type="radio" ref="color4" v-model="project.project_color_id" value="4" name="colorGroup" @click="radioDeselection(4)"><label></label></li>
								<li class="colorBox five"><input type="radio" ref="color5" v-model="project.project_color_id" value="5" name="colorGroup" @click="radioDeselection(5)"><label></label></li>
								<li class="colorBox six"><input type="radio" ref="color6" v-model="project.project_color_id" value="6" name="colorGroup" @click="radioDeselection(6)"><label></label></li>
								<li class="colorBox seven"><input type="radio" ref="color7" v-model="project.project_color_id" value="7" name="colorGroup" @click="radioDeselection(7)"><label></label></li>
								<li class="colorBox eight"><input type="radio" ref="color8" v-model="project.project_color_id" value="8" name="colorGroup" @click="radioDeselection(8)"><label></label></li>
								<li class="colorBox nine"><input type="radio" ref="color9" v-model="project.project_color_id" value="9" name="colorGroup" @click="radioDeselection(9)"><label></label></li>
								<li class="colorBox ten"><input type="radio" ref="color10" v-model="project.project_color_id" value="10" name="colorGroup" @click="radioDeselection(10)"><label></label></li>
							</ul>
						</div>
					</div>
					...
					<div class="content__input">
						...
						<div class="input__add" v-if="is_new_project_orderer">
							...
							<div class="content__input" v-if="!is_tel">
								...
								<div class="input__box">
									<input class="bgType" type="tel" name="" v-model="project_orderer.phone" :required="!is_new_project_orderer" oninput="value = value.replace(/[０-９]/g,s => String.fromCharCode(s.charCodeAt(0) - 65248)).replace(/\D/g,'');">
								</div>
							</div>
							<div class="content__confirmation">
...
<script>
	...
	export default {
		...
		mounted: function() {},
		updated: function() {
            if (this.deleteMemberId == 1) {
                this.deleteMemberId= this.charges_for_delete.length > 0 ? this.charges_for_delete[0].id : 1
            }
            console.log(this.$store.getters.getWorkModalActive, "Work")
            console.log(this.$store.getters.getPdfModalActive, "PDF")
            console.log(this.$store.getters.getWorkModalActive, "Work")
            let infoId = this.$store.state.infoId;
            var project = undefined
            if (this.isCharge) {
                project.charge_id = parseInt(this.loginId)
                console.log(this.loginId, "asdfad")
            } else {
                var selectedCharge = this.origin_charges.filter(item => {return item.name == this.user_name})[0]
                this.project.charge_id = selectedCharge ? selectedCharge.id : 0
                console.log(this.project.charge_id, "HTHH")
            }
            ...
            if (this.$store.state.calendars[infoId.calendarId][infoId.columnStatus]) {
                project = this.$store.state.calendars[infoId.calendarId][infoId.columnStatus][infoId.lineId].works[infoId.staffId];
                project.charge_id = parseInt(this.loginId);
                project.worker_id = 0;
            }
            if (project !== undefined) {
                ...
                axios.get('/api/projects/' + project.projectId, {})
                    .then(result => {
                        ...
                        this.project = {
                            name: result.name,
                            charge_id: parseInt(this.loginId),
                            worker_id: 0,
                            ...
                            project_color_id:(result.project_color_id == null || result.project_color_id == undefined) ? "" : result.project_color_id,
                        }

                        if (this.isCharge) {
                            this.project.charge_id = parseInt(this.loginId)
                        } else {
                            var selectedCharge = this.origin_charges.filter(item => {return item.name == this.user_name})[0]
                            project.charge_id = selectedCharge.id
                        }
                        ...
                        this.checkRadio = (result.project_color_id == null || result.project_color_id == undefined) ? "" : result.project_color_id;
                    })
                    .catch(result => {
                ...
                if (this.isCharge) {
                    ...
                    var selectedCharge = this.origin_charges.filter(item => {return item.name == this.user_name})[0]
                    this.project.charge_id = selectedCharge.id
                }
                // 作業員の初期表示
                // if(this.isCharge == 1) {
                //     // 担当者のページの場合
		...
		methods: {
            // Add member modal's submit handler
            addMember: function(e) {
				this.$store.dispatch('registerMember', {
                    name: this.newMember.name,
                    password: this.newMember.password,
                    phone: this.newMember.phone,
                    edit_type: this.newMember.edit_type,
                });
                this.$store.commit('toggleAddMemberModal');
                this.newMember.name = ''
            },
            deleteMember: function(e) {
                ...
                this.$store.commit('toggleDeleteMemberModal');
            },
            // ラジオボタンのcheckedを外す
            ...
            closeDeleteMemberModal: function() {
				this.$store.commit('toggleDeleteMemberModal');
			},
			...
			registerWork: function() {
				this.$store.dispatch('registerWork', this.project);
                this.$store.commit('toggleWorkModal');
				// this.$store.state.calendars.splice();
            },
            // PDFを新しいタブで開く
		...
		computed: {
            ...
            project_color_id: {
                get() {return this.$store.state.project_color_id},
            ...
            // project_color_id: function() { return this.$store.state.project_color_id },
            ...
            origin_charges:function() { return this.$store.state.chargeLists },
            charges_for_delete:function() { return this.$store.state.managerLists.filter(item => {
                var result = true
                this.$store.state.chargeLists.forEach(element => {
                    if ((element.name == item.name && element.edit_type == 1) || (element.name == item.name && element.is_user == 1)) {
                        result = false
                    }
                });
                this.deleteMemberId = result.length > 0 ? result[0].id : 1
                return result
            }) },
            deleteMemberId_pre: function() {this.deleteMemberId = this.charges_for_delete.length > 0 ? this.charges_for_delete[0].id : 1;
