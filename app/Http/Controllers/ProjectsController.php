...
class ProjectsController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        $login_id = Auth::id();
        $user_name = AuthService::getAuthUser()->name;
        $work_on_y = $request->y;
        $work_on_m = $request->m;
        $work_on_d = $request->d;
    ...
    public function edit($id)
    {
        // 案件が自分自身のユーザーIDに紐づいていない場合、元ページへリダイレクト
        $project = Project::find($id);
        $login_id = AuthService::getAuthUser()->id;
        if ($project->user_id !== $login_id) {
            return redirect()->back();
        }
        return view('projects.edit', compact('id', 'project', 'login_id'));
    }
