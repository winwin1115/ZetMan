        ...
        while ($day <= $lastDay) {
            ...
            for ($i = 0; $i < 7; $i++) {
                if ($day <= 0 || $day > $lastDay) {
                    ...
                    $erectionProject = Project::whereDate('work_on', $date)
                        ->ofUserId(AuthService::getAuthUser()->id)
                        ->ofProjectType([config('const.project.type_name.erection')])
                        ->count();
                    $disassembledProject = Project::whereDate('work_on', $date)
                        ->ofUserId(AuthService::getAuthUser()->id)
                        // ->ofProjectType([config('const.project.type_name.disassembled')])
                        ->where('project_type', 2)
                        ->count();
                    // $undisassembledProject = Project::whereDate('work_on', $date)
                    //     ->ofUserId(AuthService::getAuthUser()->id)
                    //     ->ofProjectType([config('const.project.type_name.undisassembled')])
                    //     ->count();
                    ...
                    $allProjects = Project::whereDate('work_on', $date)
                        ->ofUserId(AuthService::getAuthUser()->id)
                        ->count();
                    $allMemos = Memo::whereDate('work_on', $date)
                        ->ofUserId(AuthService::getAuthUser()->id)
                        ->count();
                    $link = '';
                    ...
                    if($allMemos) {
                        if(is_null($charge)) {
                            $this->html .= "<span class='memo-dot'></span>";
                        }
                    }
                    ...
                    $this->html .= "</div><ul><li><span class='time'>仮設 - " . $erectionProject . "</span></li><li><span class='time'>解体 - " . $disassembledProject . "</span></li></ul></a></td>";
