...
class ChargeRemark extends Model
{
    ...
    protected $fillable = ['user_id', 'staff_id', 'time_type', 'work_on', 'remarks'];
    ...
    public static function search($params)
    {
        ...
        return self::select([
                'memos.*',
                'staffs.name as member_name',
            ])
            ->leftJoin('staffs', 'memos.staff_id', '=', 'staffs.id')
            ->ofUserId($user_id)
    ...
    {
        if (blank($key)) return;
        return $query->where('memos.user_id', $key);
    }
